...
('{title}','{date_event}','{time_event}',{location},'{description}')
event(title,date_event,time_event,location,description)
location = {'loc_name':item['place']},
		'''
		sql_insert_event_template = """
			WITH ev_key AS
				(INSERT INTO event(title,date_event,time_event,description)
							VALUES('{title}','{date_event}','{time_event}','{description}')
							RETURNING id),
				cat_key AS
				...
				INSERT INTO event_media(url,type,event_id)
